...
module.exports = {
    ...
    execute(msg) {
        if (!msg.member.hasPermission('BAN_MEMBERS')) {
            Logger(msg.author.tag), "used the ban command without the proper permissions.";
        ...
        msg.mentions.members.first().ban({ reason: `Issued by ${msg.author.tag}` }).then(() => {
            ...
            Logger(msg.author.tag, `used ban command on ${taggedMember.user.tag}.`);
            ...
            if (err.name === 'DiscordAPIError' && err.message === 'Missing Permissions') {
                ...
                return Logger(msg.author.tag, `tried to ban ${taggedMember.user.tag}, but their target was an admin.`);
            }
            return Logger(msg.author.tag, `tried to ban ${taggedMember.user.tag}, but an unknown error occured.`);      
        });
    }
};